...
public class SimpleGUICustom implements ComponentListener, Listener {
    ...
    public Runner doRun(Integer commandIndex) {
        ...
        SimpleReporterCustom.SimpleCallback1 cb = new SimpleReporterCustom.SimpleCallback1(this, null, log, Verbosity.get().ordinal(), latestAlloyVersionName, latestAlloyVersion);
        SimpleReporterCustom.SimpleTask1 task = new SimpleReporterCustom.SimpleTask1();
        ...
        opt.solver = A4Options.SatSolver.get();
        task.bundleIndex = i;
    ...
    private Runner doRefreshOption() {
        ...
        try {
            ...
            //
            final A4Options.SatSolver now = A4Options.SatSolver.get();
            final JMenu sat = new JMenu("SAT Solver: "+now);
            for(A4Options.SatSolver sc:satChoices) { menuItem(sat, ""+sc, doOptSolver(sc), sc==now?iconYes:iconNo); }
            optmenu.add(sat);
            //
            ...
            if (now!= A4Options.SatSolver.MiniSatProverJNI) cmMenu.setEnabled(false);
            optmenu.add(cmMenu);
            //
            ...
            if (now!= A4Options.SatSolver.MiniSatProverJNI) cgMenu.setEnabled(false);
            optmenu.add(cgMenu);
            //
    ...
    private Runner doOptSolver(A4Options.SatSolver solver) {
        if (!wrap) solver.set();
    ...
    private final Computer enumerator = new Computer() {
        public String compute(Object input) {
            ...
            SimpleReporterCustom.SimpleCallback1 cb = new SimpleReporterCustom.SimpleCallback1(SimpleGUICustom.this, viz, log, Verbosity.get().ordinal(), latestAlloyVersionName, latestAlloyVersion);
            SimpleReporterCustom.SimpleTask2 task = new SimpleReporterCustom.SimpleTask2();
            task.filename = arg;
            try {
    ...
    private static SimInstance convert(Module root, A4Solution ans) throws Err {
        ...
        for(Sig s: ans.getAllReachableSigs()) {
            ...
            for(Sig.Field f: s.getFields())  if (!f.defined)  ct.init(f, convert(ans.eval(f)));
        }
    ...
    private static final WorkerEngineCustom.WorkerTaskCustom dummyTask = new WorkerEngineCustom.WorkerTaskCustom() {
        private static final long serialVersionUID = 0;
        public void run(WorkerEngineCustom.WorkerCallbackCustom out) { }
    };
            ...
            public synchronized void callback(Object msg) {
                if (toTry.size()==0) {
                    SwingUtilities.invokeLater(new Runnable() {
                        public void run() {
                            SimpleGUICustom.this.frame=frame;
                            SimpleGUICustom.this.finishInit(args, allowed, windowWidth);
                        }
                    });
                    return;
                }
                try {
                    mem=toTry.remove(0);
                    WorkerEngineCustom.stop();
                    WorkerEngineCustom.run(dummyTask, mem, 128, "", "", this);
                    return;
    ...
    private void finishInit(String[] args, List<Integer> initialAllowedMemorySizes, int width) {
        ...
        if (true) {
            satChoices = A4Options.SatSolver.values().makeCopy();
...
//            if (!isSat(test1)) satChoices.remove(SatSolver.BerkMinPIPE);
            satChoices.remove(A4Options.SatSolver.BerkMinPIPE);
            String test2 = Subprocess.exec(20000, new String[]{binary+fs+"spear", "--model", "--dimacs", binary+fs+"tmp.cnf"});
            if (!isSat(test2)) satChoices.remove(A4Options.SatSolver.SpearPIPE);
            if (!loadLibrary("minisat")) {
                ...
                log.logDivider();
                log.flush();
                satChoices.remove(A4Options.SatSolver.MiniSatJNI);
            }
            if (!loadLibrary("minisatprover")) satChoices.remove(A4Options.SatSolver.MiniSatProverJNI);
            if (!loadLibrary("zchaff"))        satChoices.remove(A4Options.SatSolver.ZChaffJNI);
            A4Options.SatSolver now = A4Options.SatSolver.get();
            if (!satChoices.contains(now)) {
                now= A4Options.SatSolver.ZChaffJNI;
                if (!satChoices.contains(now)) now= A4Options.SatSolver.SAT4J;
                now.set();
            }
            if (now== A4Options.SatSolver.SAT4J && satChoices.size()>3 && satChoices.contains(A4Options.SatSolver.CNF) && satChoices.contains(A4Options.SatSolver.KK)) {
